<template>
  <div>
    <vxe-grid
      ...
      :style="[dynamicHoverStyle(), { height: '100%', width: '100%' }]"
      v-bind="gridOptions"
    ...
    >
      <template
        v-slot:generalDisplay="{ row, rowIndex, $rowIndex, column, columnIndex, $columnIndex }"
      >
        ...
        <template v-else-if="column?.cellRender?.props?.type === 'button'">
          <el-button
            :type="column.cellRender.props.cellSettings(row, column)?.type || 'primary'"
            :size="column.cellRender.props.cellSettings(row, column)?.size || ''"
            :style="column.cellRender.props.cellSettings(row, column)?.style || {}"
        ...
        <template v-else>
          <div :style="column?.cellRender?.props?.cellSettings(row, column)?.style || {}">
      ...
      <!--  -->
      <template
        v-slot:actionColumn="{ row, rowIndex, $rowIndex, column, columnIndex, $columnIndex }"
      >
        <div>
...
import cusTableProps from "./props";
...
const props = defineProps(cusTableProps);
...
const vxeGridTable = ref<any>(null); //实例
...
const pagerConfig = computed(() => {
  if (props.enablePager) {
    return {
      ...
      pagerCount: typeof props.pagerCount === "number" ? props.pagerCount + 2 : 2,
...
watch(
  ...
  (val) => {
    if (vxeGridTable.value) {
      vxeGridTable.value.loadData(val);
    }
  },
  { deep: true }
);
...
function generateVxeCol(col: ObjectType) {
  ...
  if (!col.children) {
    vxeCol = {
      ...
      showOverflow: typeof col.showOverflow !== "undefined" ? col.showOverflow : true,
      showHeaderOverflow:
        typeof col.showHeaderOverflow !== "undefined" ? col.showHeaderOverflow : false,
    ...
    vxeCol = {
      ...
      showOverflow: typeof col.showOverflow !== "undefined" ? col.showOverflow : true,
      showHeaderOverflow:
        typeof col.showHeaderOverflow !== "undefined" ? col.showHeaderOverflow : true,
...
function actionStyle({
  ...
  } else if (typeof props.cellStyle === "object" && Object.keys(props.cellStyle).length > 0) {
    style = props.cellStyle;
  } else if (typeof props.rowStyle === "object" && Object.keys(props.rowStyle).length > 0) {
